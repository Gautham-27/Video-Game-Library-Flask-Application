...
class Game:
    def __init__(self, game_id: int, game_title: str):
        ...
        self.__price = None
        self.__release_date = "Jan 1, 1970"
    ...
    def add_review(self, review: 'Review'):
        # Forward reference for Review required, since both classes have to reference each other.
...
class User:
    def __init__(self, username: str, password: str):
        ...
        self.update_history("Created their account")
    ...
    def add_review(self, new_review):
        ...
        self.update_history(f"Created a {new_review.rating}/5 review for '{new_review.game.title}'")

    def remove_review(self, review):
        if not isinstance(review, Review) or review not in self.__reviews:
            return
        self.__reviews.remove(review)
        self.update_history(f"Removed a review for '{review.game.title}'")
    ...
    def add_favourite_game(self, game):
        ...
        self.update_history(f"Added '{game.title}' to favourites")
    ...
    def remove_favourite_game(self, game):
        ...
        self.update_history(f"Removed '{game.title}' from favourites")
    ...
    def add_to_wishlist(self, game: Game):
        if isinstance(game, Game) and game not in self.__wishlist:
            self.__wishlist.add_game(game)
            self.update_history(f"Added '{game.title}' to wishlist")

    def remove_from_wishlist(self, game: Game):
        if isinstance(game, Game) and game in self.__wishlist:
            self.__wishlist.remove_game(game)
            self.update_history(f"Removed '{game.title}' from wishlist")
    ...
    def update_history(self, event: str):
        if event is not None:
            self.__history.append(History(self, event))
...
class History:
    def __init__(self, user: User, entry: str):
        if not isinstance(user, User):
            raise ValueError("User must be an instance of User class")
        ...
        self.__user = user
        self.__datetimestamp = datetime_stamp()
        self.__entry = entry
    ...
    def __repr__(self):
        return f"<History Record: '{self.__entry}' at {self.__datetimestamp}>"

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        return (self.__user == other.__user and
                self.__datetimestamp == other.__datetimestamp and
                self.__entry == other.__entry)

    def __hash__(self):
        return hash(self.__entry)

    def __lt__(self, other):
        if not isinstance(other, self.__class__):
            return False
        return (datetime.strptime(self.__datetimestamp, "%d/%m/%Y %I:%M %p")
                < datetime.strptime(other.__datetimestamp, "%d/%m/%Y %I:%M %p"))


def create_review(user: User, game: Game, rating: int, comment: str):

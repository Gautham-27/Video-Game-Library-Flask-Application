import pytest
import os
from games.domainmodel.model import Publisher, Genre, Game, Review, User, Wishlist, create_review
...
def test_game_add_remove_reviews():
    user = User("Shyamli", "pw12345")
    game = Game(1, "Domino Game")
    review1 = Review(user, game, 3, "Great game!")
    review2 = Review(user, game, 4, "Superb game!")
    ...
    assert game.reviews == [review1, review2, review3]
    game.add_review(review1)
    assert len(game.reviews) == 3
    game.add_review(300)
    game.add_review('review')
    game.add_review(None)
    assert len(game.reviews) == 3
    game.remove_review(review2)
    game.remove_review(review1)
    game.remove_review(review1)
    assert game.reviews == [review3]


def test_user_wishlist_add_remove(user):
    game1 = Game(1, "Test Game")
    game2 = Game(2, "Test Quest")
    game3 = Game(3, "Unit Tests: The Game")

    user.add_to_wishlist(game1)
    assert len(user.wishlist.list_of_games()) == 1
    assert game1 in user.wishlist

    user.add_to_wishlist(game2)
    assert len(user.wishlist.list_of_games()) == 2
    assert game1 in user.wishlist
    assert game2 in user.wishlist

    user.add_to_wishlist(game3)
    user.remove_from_wishlist(game2)
    assert len(user.wishlist.list_of_games()) == 2
    assert game1 in user.wishlist
    assert game2 not in user.wishlist
    assert game3 in user.wishlist

    user.add_to_wishlist("Games?")
    user.add_to_wishlist(1337)
    user.add_to_wishlist(None)
    assert len(user.wishlist.list_of_games()) == 2
    assert game1 in user.wishlist
    assert game2 not in user.wishlist
    assert game3 in user.wishlist

    user.remove_from_wishlist("Games?")
    user.remove_from_wishlist(1337)
    user.remove_from_wishlist(None)
    assert len(user.wishlist.list_of_games()) == 2
    assert game1 in user.wishlist
    assert game2 not in user.wishlist
    assert game3 in user.wishlist

    user.remove_from_wishlist(game1)
    user.remove_from_wishlist(game3)
    assert len(user.wishlist.list_of_games()) == 0
    assert game1 not in user.wishlist
    assert game2 not in user.wishlist
    assert game3 not in user.wishlist


def test_user_history_update_history(user, game):
    assert len(user.history) == 1
    history_messages = [entry.entry for entry in user.history]
    assert "Created their account" in history_messages

    user.update_history("Tested the waters")
    history_messages = [entry.entry for entry in user.history]
    assert len(user.history) == 2
    assert "Created their account" in history_messages
    assert "Tested the waters" in history_messages

    review = Review(user, game, 5, "Good!")
    user.add_review(review)
    history_messages = [entry.entry for entry in user.history]
    assert len(user.history) == 3
    assert "Created their account" in history_messages
    assert "Tested the waters" in history_messages
    assert f"Created a {review.rating}/5 review for '{review.game.title}'" in history_messages
    ...
    assert "Tested the waters" in history_messages
    assert f"Created a {review.rating}/5 review for '{review.game.title}'" in history_messages
    assert f"Added '{game.title}' to wishlist" in history_messages
    ...
    assert f"Created a {review.rating}/5 review for '{review.game.title}'" in history_messages
    assert f"Added '{game.title}' to wishlist" in history_messages
    assert f"Removed '{game.title}' from wishlist" in history_messages

    user.remove_review(review)
    history_messages = [entry.entry for entry in user.history]
    assert len(user.history) == 6
    assert "Created their account" in history_messages
    assert "Tested the waters" in history_messages
    assert f"Created a {review.rating}/5 review for '{review.game.title}'" in history_messages
    assert f"Added '{game.title}' to wishlist" in history_messages
    assert f"Removed '{game.title}' from wishlist" in history_messages
    assert f"Removed a review for '{review.game.title}'" in history_messages
